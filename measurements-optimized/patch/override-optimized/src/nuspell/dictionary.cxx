    ...
    -> High_Quality_Sugs
{
	...
	auto retu = uppercase_suggest(word, out);
	auto tick_b = high_resolution_clock::now();
	auto dur = tick_b - tick_a;
	cerr << "uppercase_suggest " << dur.count() << '\n';
	if (!retu)
#else
	...
	retu = rep_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "rep_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = map_suggest(word, out, attempt, timeout,
	                   high_resolution_clock::now());
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "map_suggest " << dur.count() << '\n';
	cerr << "map_suggest_attempts " << attempt << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = adjacent_swap_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "adjacent_swap_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = distant_swap_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "distant_swap_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = keyboard_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "keyboard_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = extra_char_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "extra_char_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = forgotten_char_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "forgotten_char_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = move_char_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "move_char_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = bad_char_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "bad_char_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = doubled_two_chars_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "doubled_two_chars_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = two_words_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "two_words_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
	tick_a = high_resolution_clock::now();
	retu = phonetic_suggest(word, out);
	tick_b = high_resolution_clock::now();
	dur = tick_b - tick_a;
	cerr << "phonetic_suggest " << dur.count() << '\n';
	if (!retu)
#else
...
#ifdef TIMING
			if (out.size() >= MAX_SUGGEST) {
				cerr << "forgotten_char_suggest_attempts "
				     << attempt << '\n';
				return false;
			}
...
#ifdef TIMING
	cerr << "forgotten_char_suggest_attempts " << attempt << '\n';
#endif
	return true;
}
...
#ifdef TIMING
			if (out.size() >= MAX_SUGGEST) {
				cerr << "bad_char_suggest_attempts " << attempt
				     << '\n';
				return false;
			}
			if ((++attempt % MAX_ATTEMPT == 0) &&
