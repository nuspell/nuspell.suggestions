...
auto print_help(const string& program_name) -> void
{
    ...
    o << "\n"
         ...
         "  Speedup\n"
         "\n"
         "All durations are in nanoseconds. Even on the same machine, timing\n"
         "can vary considerably in the second significant decimal. Use only a\n"
         "production build executable with optimizations. A speedup of 1.62\n"
         "means Nuspell is 1.6 times faster than Hunspell.\n"
         "\n"
         "Verification will be done on suggestions when a corrections file is\n"
         "provided. Each line in that file contains a unique incorrect word,\n"
         "a tab character and the most desired correct suggestions. Note that\n"
         "the second word needs to be incorrect for Nuspell and Hunspell. The\n"
         "correction should be correct for Nuspell and Hunspell.\n"
         "\n"
         "The same statistics as above will be report followed by statistics\n"
         "on the suggestions, begin:\n"
                ...
                locale& hloc, bool print_false = false, bool test_sugs = false)
{
    ...
    spell_report(out, total, true_pos, true_neg, false_pos, false_neg,
                 duration_nu_tot.count(), duration_hun_tot.count(),
                 duration_nu_min.count(), duration_hun_min.count(),
                 duration_nu_max.count(), duration_hun_max.count(),
                 speedup_max);
}
                  ...
                  bool print_sug = false)
{
    ...
    while (getline(in, sug_line)) {
...
#ifdef TIMING
        cerr << "word_suggest " << word << endl;
        cerr << "priv_suggest " << sug_duration_nu.count() << endl;
        cerr << "priv_suggest_size " << sugs_nu.size() << endl;
#endif
        ...
        if (!sugs_nu.empty() && !sugs_hun.empty() && sugs_nu[0] == sugs_hun[0])
            ++sug_same_first;
    ...
    if (sug_duration_nu_tot.count() == 0) {
        cerr << "ERROR: Invalid duration of 0 nanoseconds for Nuspell suggestions\n";
        return;
    }
    if (sug_duration_hun_tot.count() == 0) {
        cerr << "ERROR: Invalid duration of 0 nanoseconds for Hunspell suggestions\n";
        return;
    }
    ...
    auto sug_speedup = sug_duration_hun_tot.count() * 1.0 / sug_duration_nu_tot.count();
    ...
    out << "Total Duration Suggestions Nuspell      " << sug_duration_nu_tot.count() << '\n';
    out << "Total Duration Suggestions Hunspell     " << sug_duration_hun_tot.count() << '\n';
    out << "Minimum Duration Suggestions Nuspell    " << sug_duration_nu_min.count() << '\n';
    out << "Minimum Duration Suggestions Hunspell   " << sug_duration_hun_min.count() << '\n';
    out << "Average Duration Suggestions Nuspell    " << sug_duration_nu_tot.count() / sug_total
        << '\n';
    out << "Average Duration Suggestions Hunspell   " << sug_duration_hun_tot.count() / sug_total
        << '\n';
    out << "Maximum Duration Suggestions Nuspell    " << sug_duration_nu_max.count() << '\n';
    out << "Maximum Duration Suggestions Hunspell   " << sug_duration_hun_max.count() << '\n';
    out << "Maximum Suggestions Speedup             " << sug_speedup_max << '\n';
    out << "Suggestions Speedup                     " << sug_speedup << '\n';

    if (!sug_excluded.empty()) {
        out << "The following words are correct and should not be used:\n";
